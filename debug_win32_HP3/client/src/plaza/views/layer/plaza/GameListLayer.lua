...
function GameListLayer:ctor(gamelist, gameClass)
	print("============= 游戏列表界面创建 =============")
    ...
    for i, v in pairs(gamelist) do
        local gameinfo = v
        if gameinfo then
            if (gameinfo._GameClass == gameClass) or (gameinfo._GameClass == nil and gameClass == yl.GAMECLASS_CARD) then
                table.insert(self._gameList, gameinfo)
            end
        end
	end
	...
	self._listView:setPosition(cc.p(0,100))
...
function GameListLayer:onEnterGame( gameinfo, isQuickStart)
	...
	elseif PriRoom and true == PriRoom:getInstance():onLoginEnterRoomList() then
		...
		if GlobalUserItem.dwLockKindID == GlobalUserItem.nCurGameKind and nil ~= lockRoom then
            ...
            local clientScene = self:getParent():getParent()
			--百人场快速开始
            if (tonumber(kindId) == 140) or (tonumber(kindId) == 122) or (tonumber(kindId) == 104) or (tonumber(kindId) == 123) then
                local entergame = self:getGameInfoByKindID(tonumber(kindId))
...
function GameListLayer.tableCellAtIndex(view, idx)
	...
	local mask = nil
	local spTip = nil
	local cellpos = cc.p(view:getParent().m_fThird * 0.5,view:getViewSize().height * 0.5 + 80)
	...
	if not cell then
		...
		game:addTo(cell)
			...
			:setPosition(view:getParent().m_fThird * 0.5, 80)
			:setTag(1)
		...
		if nil ~= maskSp then
			...
			mask:setPosition(cc.p(view:getParent().m_fThird * 0.5, 80))
