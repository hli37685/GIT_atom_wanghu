...
function CheckinLayer:ctor(scene)
    ...
    -- 背景
    local spriteMainBg = cc.Scale9Sprite:create("Checkin/dialogframe.png")
    spriteMainBg:setCapInsets(CCRectMake(100,100,1134,550))
    ...
    local line1StartPosX =  450
    local line2StartPosX =  330
    local ItemIntervalX = 220
    local ItemIntervalY = 220
	...
	local dbbg=cc.Scale9Sprite:create("Checkin/lvbtn.png")
		:setCapInsets(CCRectMake(27,27,130,1))
   		:setContentSize(cc.size(170, 57))
		:setPosition(yl.WIDTH-300,100)
		:addTo(self)

	local newDBBtnSize = dbbg:getContentSize()
	ccui.Button:create("Checkin/dibao.png","Checkin/dibao.png")
		:move(newDBBtnSize.width/2, newDBBtnSize.height/2+5)
...
function CheckinLayer:setCheckinBtn(enable, hide)
    ...
    local bg = enable and "Checkin/lanlvbtn.png" or "Checkin/graybtn.png"
	local botton = enable and "Checkin/bt_checkin.png" or "Checkin/bt_checkined.png"
    if bg then
        self.m_btnCheckinBg = cc.Scale9Sprite:create(bg)
        self.m_btnCheckinBg:setCapInsets(CCRectMake(20,10,66,27))
        self.m_btnCheckinBg:setPosition(yl.WIDTH/2,110)
	    ...
	    ccui.Button:create(botton,botton)
