...
function ShopLayer:ctor(scene, stmod,gameFrame,curtag)
    ...
    --左侧背景
    --[[
    cc.Scale9Sprite:create("Shop/frame_left.png")
        :setCapInsets(CCRectMake(10,10,165,267))
        :setContentSize(cc.size(185, 492))
        :move(261, 366)
        :addTo(self)
    --]]
    --[[
    ...
    ccui.Button:create("Shop/youxibiwenzi.png","Shop/youxibiwenzi.png")
		:move(zsBgSize.width/2, zsBgSize.height/2)
		:addTo(zsBg)
    --]]

    local cbPositionY = 580
    local cbPositionXStart = 534
    local cbIntervalX = 266

    --==============================================================特权商城    不同体系
    ccui.CheckBox:create("Shop/btn.png","","Shop/btn_on.png","","")
		:move(cbPositionXStart+cbIntervalX,cbPositionY)
		:addTo(self)
		:setSelected(false)
        :setVisible(true)
        :setEnabled(true)
	...
	--游戏豆
    ccui.CheckBox:create("Shop/btn.png","","Shop/btn_on.png","","")
		:move(cbPositionXStart,cbPositionY)
		:addTo(self)
		:setSelected(true)
        :setVisible(true)
        :setEnabled(true)
    ...
    spriteItemBg_bean:setCapInsets(CCRectMake(25,22,200,0))
    spriteItemBg_bean:setContentSize(cc.size(400, 35))
    spriteItemBg_bean:move(yl.WIDTH-230,yl.HEIGHT-60)
	...
	cc.LabelTTF:create(string.formatNumberThousands(GlobalUserItem.lUserInsure,true,","), "fonts/round_body.ttf", 25) 
	...
	self._scrollView = ccui.ScrollView:create()
        :setContentSize(cc.size(1000,520))
        :setAnchorPoint(cc.p(0.5, 0.5))
--		:setPosition(cc.p(805, 314))
        :setPosition(cc.p(yl.WIDTH/2, yl.HEIGHT/2-80))
        --:setDirection(cc.SCROLLVIEW_DIRECTION_HORIZONTAL) SCROLLVIEW_DIRECTION_BOTH SCROLLVIEW_DIRECTION_VERTICAL
...
print("tag== self._select===",tag,self._select)
    ...
    local cellWidth = 800/3
    local cellWidthC2 = cellWidth/2
	if #theList<7 then
        scrollHeight=520
		self._scrollView:setInnerContainerSize(cc.size(800, 520))
	else
		scrollHeight = cellHeight * math.ceil(#theList / 3) --math.floor((#theList+math.floor(#theList%3))/3) 下部分为320 第一第二排高度和为290*2
		scrollWidth = cellWidth * math.ceil(#theList / 3)
	...
	for i=1,11 do
        ...
        --]]
        --特权商城 排除数据4 红钻
        if bPrivilege and item.cbMemberOrder and item.cbMemberOrder==4 then
        else
            local temp_i
        ...
        if true then
            --三栏位
            self._showList[i] = cc.LayerColor:create(cc.c4b(100, 100, 100, 0), cellWidthC2, 240)
                :move(215+92/2-25*2+300*((temp_i-1)%3)-130,120+scrollHeight-350-cellHeight*(math.ceil(temp_i / 3)-1))
                :addTo(self._scrollView)
            ...
            -- showSp = display.newSprite("Shop/icon_shop_5.png")
                ...
                --金币数量 价格 背景
                local SContentBg = cc.Scale9Sprite:create("Shop/frame_shop_14.png")
                SContentBg:setCapInsets(CCRectMake(43,20,101,2))
                --SContentBg:setContentSize(cc.size(190, 68))
                SContentBg:setPosition(130,80)
                self._showList[i]:addChild(SContentBg)

                local atlas = cc.LabelAtlas:_create(string.gsub(item.name .. "", "[.]", "/"), "Shop/num_shop_6.png", 15, 23, string.byte("/"))
                ...
                atlas:setPosition(130 + (atlas:getContentSize().width - wid), 80)
                name:setPosition(atlas:getPositionX(), 80)

                --按钮背景
                local priceContentBg = cc.Scale9Sprite:create("Shop/srkdt.png")
                priceContentBg:setCapInsets(CCRectMake(35,28,100,1))
                --priceContentBg:setContentSize(cc.size(190, 68))
                priceContentBg:setPosition(130,20)
                self._showList[i]:addChild(priceContentBg)

                --按钮
                local price1 = cc.LabelAtlas:_create(string.gsub(item.price .. "", "[.]", "/"), "Shop/num_shop_7.png", 22, 28, string.byte("/"))
                price1:setAnchorPoint(cc.p(1.0,0.5))
                price1:setScale(1)
                self._showList[i]:addChild(price1)
                local price2 = display.newSprite("Shop/yuan.png")
                price2:setScale(1)
                price2:setAnchorPoint(cc.p(0,0.5))
                self._showList[i]:addChild(price2)
                local wid = (price1:getContentSize().width + price2:getContentSize().width) / 2   			
                price1:setPosition(130 + (price1:getContentSize().width - wid), 24)
                price2:setPosition(price1:getPositionX(), 24)
            ...
            elseif bPrivilege then
                ...
                local atlas = cc.LabelAtlas:_create(string.gsub(item.lMemberPrice/10000 .. "", "[.]", "/"), "Shop/num_shop_6.png", 15, 23, string.byte("/"))
                atlas:setAnchorPoint(cc.p(1.0,0.5))
                self._showList[i]:addChild(atlas)
                ...
                --按钮
                local price1 = display.newSprite("Shop/buy.png")
                price1:setScale(1)
                price1:setAnchorPoint(cc.p(0.5,0.5))
                self._showList[i]:addChild(price1)
                price1:setPosition(130, 24)
            else
                --[[
            ...
            if nil ~= showSp then
                showSp:setPosition(130, 150)
                if bPrivilege then
                    showSp:setPosition(130, 140)
                end 
            ...
            --[[
            if nil ~= sign and priceLabel then     
                local width = 0  		
