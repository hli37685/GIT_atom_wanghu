...
local GameClassification = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.GameClassification")
...
function ClientScene:onEnterTransitionFinish()
	...
	--[[
	--返回动画
    self.m_actShopAni = cc.CSLoader:createTimeline("Lobby/fanhui.csb")
	ExternalFun.SAFE_RETAIN(self.m_actShopAni)
	self._rotate:stopAllActions()
	self.m_actShopAni:gotoFrameAndPlay(0,true)
	self._rotate:runAction(self.m_actShopAni)
	--]]

	--重新取得游戏信息结束后请求公告
	--self:repeatGetGamelist()
...
function ClientScene:onCreate()
	...
	local Btn_top = csbNode:getChildByName("top")
	self._Btn_top = Btn_top

    --查看玩家信息
	self._btPersonInfo = Btn_top:getChildByName("Btn_Person")
    ...
    self._btPersonInfo:loadTextureDisabled(_personInfo)
    self._btPersonInfo:loadTextureNormal(_personInfo)
    self._btPersonInfo:loadTexturePressed(_personInfo)  
    ...
    local nickname = string.stringEllipsis(GlobalUserItem.szNickName,self._enSize,self._cnSize,200)

    self.mNickname = Btn_top:getChildByName("Text_Nickname")
	self.mNickname:setString(nickname)

	----vip信息
   	self.mLevel = Btn_top:getChildByName("Img_LvNum")
	local level=GlobalUserItem.cbMemberOrder
	if level and level >= 1 and level <=5 then
		self.mLevel:loadTexture("Lobby/VIP"..GlobalUserItem.cbMemberOrder..".png")
		self.mLevel:setVisible(true)
	elseif  level and level ==0 then
		self.mLevel:setVisible(false)
	end
	
	local coinnum = Btn_top:getChildByName("Image_coinnumbg")
	...
	local Btn_bottom = csbNode:getChildByName("bottom")
	self._Btn_bottom = Btn_bottom

    --设置
	self._btSetting = Btn_bottom:getChildByName("Btn_Setting")
    self._btSetting:setTag(ClientScene.BT_CONFIG)
    self._btSetting:addTouchEventListener(btcallback)

	--银行
	btn = Btn_bottom:getChildByName("Btn_Bank")
	btn:setTag(ClientScene.BT_BANK)
	btn:addTouchEventListener(btcallback)

	--客服
	btn = Btn_bottom:getChildByName("Btn_KeFu")
	btn:setTag(ClientScene.BT_KEFU)
	btn:addTouchEventListener(btcallback)
	self.m_btKeFu = btn;

	--背包
	btn = Btn_bottom:getChildByName("Btn_Bag")
	btn:setTag(ClientScene.BT_BAG)
	btn:addTouchEventListener(btcallback)
	self.m_btBag = btn;

	--商城
	btn = Btn_bottom:getChildByName("Btn_Shop")
	btn:setTag(ClientScene.BT_SHOP)
	btn:addTouchEventListener(btcallback)

	--返回
	self._btExit = Btn_bottom:getChildByName("Btn_Back")
	self._btExit:setTag(ClientScene.BT_EXIT)
	self._btExit:addTouchEventListener(btcallback)

    --[[
	--动画
	local rotate = ExternalFun.loadCSB("Lobby/fanhui.csb", self._btExit)
			:setPosition(self._btExit:getContentSize().width/2,self._btExit:getContentSize().height/2)
	self._rotate = rotate
	--]]

    --签到有礼
	self._btQDYL = Btn_bottom:getChildByName("btn_qdyl")
...
--	btn:setTag(ClientScene.BT_TRUMPET)
	...
	if 1 == #self:getApp()._gameList and yl.SINGLE_GAME_MODOLE then
		...
		if nil ~= entergame then
			...
			self:onChangeShowMode(yl.SCENE_GAMECLASS)
		end	
	else
		self:onChangeShowMode(yl.SCENE_GAMECLASS)
	end
...
function ClientScene:onKeyBack(tag, param)
	...
	self:onChangeShowMode(tag, param)
end

function ClientScene:onKeyBackGameList(tag)
    --self:onKeyBack(yl.SCENE_GAMELIST)
    self:onKeyBack(yl.SCENE_GAMELIST, self:getGameListParam())
end
...
function ClientScene:onButtonClickedEvent(tag,ref)
	...
	elseif tag == ClientScene.BT_QUICKSTART then
		...
		--游戏列表
		if curScene == yl.SCENE_GAMELIST or curScene == yl.SCENE_GAMECLASS then		
			self:quickStartGame()
...
function ClientScene:getGameListParam()
    return self._GameListParam
end
...
function ClientScene:onChangeShowMode(nTag, param)
	...
	--旧头部底部动画替换
	self._Btn_bottom:stopAllActions()
	--self._btKSKS:stopAllActions()
	--self._AdvFrame:stopAllActions()
	--
	if tag == yl.SCENE_GAMELIST or tag == yl.SCENE_ROOMLIST  or tag == yl.SCENE_ROOM  or tag == yl.SCENE_GAMECLASS then
		self._Btn_bottom:runAction(cc.MoveTo:create(0.3,cc.p(667,0)))
	else
		if PriRoom and PriRoom.haveBottomTop(tag) then
			if self._Btn_bottom:getPositionY() < 0 then
				self._Btn_bottom:runAction(cc.MoveTo:create(0.3,cc.p(667,0)))
			end
		else
			if tag == yl.SCENE_GAME then
				self._Btn_bottom:setPosition(cc.p(667,-190))
			else
				--self._Btn_bottom:runAction(cc.MoveTo:create(0.3,cc.p(667,-190)))
			end
		end				
	end

    --控制商城动画 新加 SCENE_ROOMLIST场景
	local infoShow = (tag == yl.SCENE_GAMELIST or bRoomList or tag == yl.SCENE_ROOM or tag == yl.SCENE_ROOMLIST  or tag == yl.SCENE_GAMECLASS )
	if true == infoShow then
		if nil ~= self.m_actShopAni and nil~=self._rotate then
			self._rotate:stopAllActions() -- 会停止返回动画 暂时注释
			--[[
			self.m_actShopAni:gotoFrameAndPlay(0,true) --->gotoFrameAndPlay(0,60,true);//从第0帧到60帧循环播放。还有其他重载函数，具体看源码。 
			self._rotate:runAction(self.m_actShopAni)
			--]]
		end
	end
	...
	--控制宝箱、喇叭显示	
	local infoShow = (bRoomList or tag == yl.SCENE_ROOM  or tag == yl.SCENE_GAMELIST or tag == yl.SCENE_GAMECLASS)
	self._notify:setVisible(infoShow)

    --控制幸运转盘、签到有礼、快速开始显示   待添加 排行榜
	--self._btQDYL:setVisible(infoShow)
	--控制返回 显示
	local infoShow = (bRoomList or tag == yl.SCENE_ROOM  or tag == yl.SCENE_GAMELIST )
    self._btExit:setVisible(infoShow)
	...
	--更新金币
	infoShow = (tag == yl.SCENE_GAMELIST or tag == yl.SCENE_ROOMLIST or tag == yl.SCENE_ROOM or bRoomList or tag == yl.SCENE_GAMECLASS)
	if true == infoShow then
		self:updateInfomation()
	end
...
function ClientScene:getTagLayer(tag, param,curtag)
	local dst
	if tag == yl.SCENE_GAMELIST then
		dst =  GameListView:create(self:getApp()._gameList, param)
 --       dst:setLocalZOrder(-2000)
    elseif tag == yl.SCENE_GAMECLASS then
		dst = GameClassification:create(self)
	elseif tag == yl.SCENE_ROOMLIST then
		--是否有自定义房间列表
		local entergame = self:getEnterGameInfo()
		if nil ~= entergame then
...
function ClientScene:quickStartGame()	
    ...
    local gameClass = self._sceneLayer:getChildByTag(yl.SCENE_GAMECLASS)
	local gameinfo = self:getEnterGameInfo()
	if (nil ~= gamelist or nil ~= gameClass) and nil ~= gameinfo then
		--进入游戏第一个房间
...
-- 刷新头像 昵称 -个人中心
function ClientScene:Refresh_head_nickname()
	local Re_Gender = (GlobalUserItem.cbGender == yl.GENDER_MANKIND and true or false)
	print("===================-=-=-=-=Refresh_head_nickname",Re_Gender,GlobalUserItem.szNickName)
    local nickname = string.stringEllipsis(GlobalUserItem.szNickName,self._enSize,self._cnSize,200)
	self.mNickname:setString(nickname)
	local _personInfo="Lobby/tx1.png"
	if Re_Gender then
...
--重新请求游戏列表
function ClientScene:repeatGetGamelist()
	local this = self
	...
	local vcallback = function(datatable)
                ...
                --返回结果
	 		    ...
	 		    if succeed == true then	 
 				    ...
 				    for k,v in pairs(rows) do
                        ...
                        gameinfo._GameClass = tonumber(v["TypeID"])						
 	...
 	appdf.onHttpJsionTable("http://ww1.16yi.com:8080/WS/iosgamelist5.txt","get","action=getgamelist&versionID=99",vcallback)--ethj 这里要改，
end
