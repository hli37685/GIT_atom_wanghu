...
local PasswordView = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.logon.PasswordView")
...
LogonScene.BT_LONGON 			= 3
LogonScene.BT_ERGISTER			= 4
LogonScene.BT_WEIXIN			= 5
...
function LogonScene:onCreate()
	...
	local  newbtncallback = function(ref, type)
        if type == ccui.TouchEventType.ended then
         	this:onNewButtonClickedEvent(ref:getTag(),ref)
        end
    end
	...
	--背景
    display.newSprite("public/background_1.png")
...
--			:addTouchEventListener(btcallback)
	...
	self._btQUIT:setTag(LogonScene.BT_EXIT)
	...
	--登录按钮
	local btnZhuceBg = display.newSprite("Logon/lvbtn.png")
		:setPosition(yl.WIDTH/2 - 150 ,200)
		:setScale(1)
		:setName("Parentbtn_1")
		:addTo(self)
	...
	ccui.Button:create("Logon/denglu.png","Logon/denglu.png")
		...
		:setTag(LogonScene.BT_LONGON)
		:addTo(btnZhuceBg)
		:addTouchEventListener(newbtncallback)

	--微信按钮
    local btnZhuceBg = display.newSprite("Logon/lanlvbtn.png")
		:setPosition(yl.WIDTH/2 + 150 ,200)
		:setScale(1)
		:setName("Parentbtn_2")
		:addTo(self)
	...
	ccui.Button:create("Logon/weixin.png","Logon/weixin.png")
		:move(newBtnSize.width/2, newBtnSize.height/2)
		:setTag(LogonScene.BT_WEIXIN)
		:addTo(btnZhuceBg)
		:addTouchEventListener(newbtncallback)

	--注册按钮
	ccui.Button:create("Logon/regtext.png","Logon/regtext.png")
		:setPosition(yl.WIDTH/2 + 300 ,300)
		:setTag(LogonScene.BT_ERGISTER)
		:addTo(self)
		:addTouchEventListener(newbtncallback)
...
--是否隐藏微信登录按钮
function LogonScene:isWXPInstalled()
	--平台判定
	local targetPlatform = cc.Application:getInstance():getTargetPlatform()
	if (cc.PLATFORM_OS_IPHONE == targetPlatform) or (cc.PLATFORM_OS_IPAD == targetPlatform) or (cc.PLATFORM_OS_ANDROID == targetPlatform) then
		...
		if true == isInstalled then
			...
			self:getChildByName("Parentbtn_1"):setPosition(cc.p(yl.WIDTH/2, 200))
		end
	else
		--隐藏
		print("隐藏微信登录按钮===========2")		
...
--新加按钮时间 登录注册
function LogonScene:onNewButtonClickedEvent(tag,ref)
	print("",tag)
	if tag==LogonScene.BT_LONGON then
		--登录
		self:onShowLogon(false)
	elseif tag==LogonScene.BT_ERGISTER then
		--注册
		self:onShowRegister()
	elseif tag==LogonScene.BT_WEIXIN then
		--微信登录
		--平台判定
		local targetPlatform = cc.Application:getInstance():getTargetPlatform()
		if (cc.PLATFORM_OS_IPHONE == targetPlatform) or (cc.PLATFORM_OS_IPAD == targetPlatform) or (cc.PLATFORM_OS_ANDROID == targetPlatform) then
...
--退出登录页面
function LogonScene:onLogonSignOut()
	--取消登录界面
	if self._logonView ~= nil then
		self._logonView:setPosition(cc.p(yl.WIDTH,0))
	end
end
--退出注册页面
function LogonScene:onRegisterSignOut()
	--取消注册界面
	if self._registerView ~= nil then
		self._registerView:setPosition(cc.p(-yl.WIDTH,0))
	end
end
...
function LogonScene:onShowLogon(bCheckAutoLogon)
    ...
    --取消忘记密码界面
    if self._findPasswordView ~= nil then
        --self._findPasswordView:runAction(cc.MoveTo:create(0.3,cc.p(-yl.WIDTH,0)))
		self._findPasswordView:setPosition(cc.p(-yl.WIDTH,0))
    end
	...
	bCheckAutoLogon = false
	if not bCheckAutoLogon then
		self._logonView:setPosition(cc.p(0,0))
		this._logonView:onReLoadUser()			
	else
		self._logonView:setPosition(cc.p(0,0))
		this._logonView:onReLoadUser()
...
function LogonScene:onShowRegister()
	...
	if self._serviceView ~= nil then
		...
		self._serviceView:setPosition(cc.p(yl.WIDTH,0))
	end
	...
	self._registerView:setPosition(cc.p(0,0))
end

--显示忘记密码界面
function LogonScene:onShowFindPassword()
	--取消登录界面
	if self._logonView ~= nil then
		self._logonView:runAction(cc.MoveTo:create(0.3,cc.p(yl.WIDTH,0)))
	end
    ...
    if self._findPasswordView == nil then
        self._findPasswordView = PasswordView:create(self)
            :move(-yl.WIDTH,0)
			:addTo(self._backLayer)
    else
        self._findPasswordView:stopAllActions()
    end

    self._findPasswordView:runAction(cc.MoveTo:create(0.3,cc.p(0,0)))
end
...
--注册账号
function LogonScene:onRegister(szAccount,szPassword,szRePassword,bAgreement,szSpreader,szNickName,szYanzhengma,szPhone)
	--输入检测
    ...
    -- 昵称非法检测
	len = ExternalFun.stringLen(szNickName)--#szAccount
	if len < 6 or len > 31 then
		showToast(self,"游戏昵称必须为6~31个字符，请重新输入！",2,cc.c4b(250,0,0,255));
		return
	end

	--判断emoji
    if ExternalFun.isContainEmoji(szNickName) then
        showToast(self, "昵称包含非法字符,请重试", 2)
        return
    end

	--判断是否有非法字符
	if true == ExternalFun.isContainBadWords(szNickName) then
		showToast(self, "昵称中包含敏感字符,不能注册", 2)
		return
	end
	...
	self:showPopWait()
	self._Operate = 1
	self._logonFrame:onRegister(szAccount,szPassword, self._gender,szSpreader,szNickName,szYanzhengma,szPhone)
end
				...
				if type(jstable) == "table" then
					...
					if type(data) == "table" then
						...
						if valid then
                            ...
                            GlobalUserItem.szSpreaderURL = "http://xz.2bai.org/14046398" -- add by lqy
							...
							GlobalUserItem.szWXSpreaderURL = "http://xz.2bai.org/14046398"
