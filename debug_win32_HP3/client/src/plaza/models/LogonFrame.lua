...
function LogonFrame:onRegister(szAccount,szPassword,cbGender,szSpreader,szNickName,szYanzhengma,szPhone)
	--数据保存
	self._szRegAccount = szAccount
    self._szNickName = szNickName
	self._szRegPassword= szPassword
	self._cbRegGender = cbGender
	self._szMobilePhone = szPhone
	self._szSpreader = szSpreader
    self._szYanzhengma = szYanzhengma
...
function LogonFrame:sendRegister()
	...
	RegisterData:pushstring(self._szNickName,yl.LEN_NICKNAME)
   ...
   -- RegisterData:pushstring(self._szYanzhengma,yl.LEN_PHONE_CODE)
