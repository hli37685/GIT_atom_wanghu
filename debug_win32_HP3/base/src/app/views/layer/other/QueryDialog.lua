...
--		:addTo(self)
    local maskLayer = CCLayerColor:create(ccc4(0, 0, 0, 120), appdf.WIDTH,appdf.HEIGHT)
    maskLayer:setOpacity(70) 
    self:addChild(maskLayer) 

    -- 背景
    self.spriteMainBg = cc.Scale9Sprite:create("public/dialogframe-1.png")
    self.spriteMainBg:setCapInsets(CCRectMake(311,184,20,26))
    self.spriteMainBg:setContentSize(cc.size(757, 450))
    self.spriteMainBg:setTag(QueryDialog.DG_QUERY_EXIT)
    self.spriteMainBg:setPosition(appdf.WIDTH/2,appdf.HEIGHT/2)
    self:addChild(self.spriteMainBg)

	--标题背景
    self.spriteContentBg = cc.Scale9Sprite:create("public/frame.png")
    self.spriteContentBg:setCapInsets(CCRectMake(40,40,560,392))
    self.spriteContentBg:setContentSize(cc.size(700, 270))
    self.spriteContentBg:move(appdf.WIDTH/2,appdf.HEIGHT-390)
    self:addChild(self.spriteContentBg)

	--标题背景
	self.title = display.newSprite("public/title_frame.png")
		:move(appdf.WIDTH/2,appdf.HEIGHT-185)
		:addTo(self)
	--标题
	self.title = display.newSprite("public/sysmsgtitle.png")
		:move(appdf.WIDTH/2,appdf.HEIGHT-185)
		:addTo(self)

	--返回
	ccui.Button:create("public/closebtn-1.png","public/closebtn-1.png")
    	:move(990,appdf.HEIGHT-200)
...
--	end
    ...
    local btnOKBg = display.newSprite("public/lanlvbtn.png")
		:move(appdf.WIDTH/2 , 230 )
		:addTo(self)
    local newBtnSize = btnOKBg:getContentSize()
	ccui.Button:create("public/btn_ok.png", "public/btn_ok.png")
