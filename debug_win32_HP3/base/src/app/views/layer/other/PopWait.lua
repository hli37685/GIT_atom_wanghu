...
function PopWait:ctor(isTransparent,width,height,posx,poxy,correct)
    ...
    local PosXm=appdf.WIDTH/2
    local PoxYm=appdf.HEIGHT/2
    local sPosXm=0
    local sPoxYm=0
    if nil~=posx then
        PosXm=posx
    ...
    if nil~=correct then
        self:move(sPosXm,sPoxYm+correct)
     else
        self:move(sPosXm,sPoxYm)
    end
