...
--local URL_REQUEST = "http://ry.webfox.com" --@http_url
local URL_REQUEST = "http://vip.qqlgame.com:9002" --@http_url
...
--	end
	...
	if nil ~= sp then
		sp = cc.Sprite:create(sloganfile)
		sp:setPosition(appdf.WIDTH/2, 400)
        sp:setVisible(false)
		self:addChild(sp)
	end
...
function WelcomeScene:httpNewVersion()	
	 		    ...
	 		    if succeed == true then	 
					...
					if nNewV and nCurV then
 				    ...
 				    for k,v in pairs(rows) do
						...
						print(" WelcomeScene:httpNewVersion gameinfo._KindID2:" .. gameinfo._KindID)
						...
						--typeID 在乐呗上的base中未取 需要在client中重新取得
                        gameinfo._GameClass = tonumber(v["TypeID"])						


						--gameinfo._KindVersion=101056515
						--gameinfo._ServerResVersion = 0
	...
	--appdf.onHttpJsionTable(URL_REQUEST .. "/WS/MobileInterface.ashx","get","action=getgamelist&versionID=2",vcallback)
	--  appdf.onHttpJsionTable(URL_REQUEST .. "/WS/MobileInterface.ashx","get","action=getgamelist&versionID=99",vcallback)
 ...
 print("地址： http://ww1.16yi.com:8080/WS/iosgamelist5.txt")
 --appdf.onHttpJsionTable("http://lua.16yi.com:8080/WS/gamelist1.txt","get","action=getgamelist&versionID=99",vcallback)
