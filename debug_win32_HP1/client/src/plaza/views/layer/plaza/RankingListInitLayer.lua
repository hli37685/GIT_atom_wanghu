local RankingListinitLayer = class("RankingListinitLayer", function(scene)
		local RankingListinitLayer = display.newLayer(cc.c4b(0, 0, 0, 255))
    return RankingListinitLayer
end)
local PopupInfoHead = appdf.req(appdf.EXTERNAL_SRC .. "PopupInfoHead")

--继续游戏
RankingListinitLayer.BT_CONTINUE = 101

-- 进入场景而且过渡动画结束时候触发。
function RankingListinitLayer:onEnterTransitionFinish()
---[[
...
-- 退出场景而且开始过渡动画时候触发。
---[[
function RankingListinitLayer:onExitTransitionStart()
    return self
end
--]]

function RankingListinitLayer:ctor(scene, preTag)
	showToast(self, "打包测试---, RankingListinitLayer.ctor", 4)
	local this = self
	self.m_bRequestData = false
...
function RankingListinitLayer:onUpdateShow()
...
function RankingListinitLayer:onButtonClickedEvent( tag, sender )
	if RankingListinitLayer.BT_CONTINUE == tag then
...
function RankingListinitLayer:onKeyBack()
...
--子视图大小
function RankingListinitLayer.cellSizeForTable(view, idx)
  	return 369-8 , 70
end

--子视图数目
function RankingListinitLayer.numberOfCellsInTableView(view)
...
--获取子视图
function RankingListinitLayer.tableCellAtIndex(view, idx)	
...
--	end
	...
	if item.lScore>=100000000 then
		local money = string.stringEllipsis(math.floor(item.lScore/100000000),_enSize,_cnSize,115)
...
function RankingListinitLayer.tableCellTouched(view, cell)
	if nil ~= cell then
