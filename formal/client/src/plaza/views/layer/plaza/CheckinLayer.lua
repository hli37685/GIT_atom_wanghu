...
--			end)
    ...
    spriteContentBg:setContentSize(cc.size(1000, 450))
    spriteContentBg:setPosition(yl.WIDTH/2, yl.HEIGHT/2 + 15)
...
--	self._btConfig:setEnabled(GlobalUserItem.wSeriesDate == 0)
    ...
    self:setCheckinBtn(GlobalUserItem.wSeriesDate == 0)

    local line1StartPosX =  440
    local line2StartPosX =  290
    local ItemIntervalX = 250
    local ItemIntervalY = 200
    local line1StartPosY = 500
	--签到展示区域
	for i=0,CheckinLayer.CHECKIN_NUMBER-1 do
        local indexY = (i < 3) and 0 or 1
        local indexX = i - 3*indexY 
        local itemPosX = ((indexY == 0) and line1StartPosX or line2StartPosX) + indexX * ItemIntervalX
        local itemPosY = line1StartPosY - ItemIntervalY * indexY
        print("=======CheckinLayer:ctor "..i.." -- ("..itemPosX..","..itemPosY..")".."indexY:"..indexY.."indexX:"..indexX)
		-- 签到背景
		display.newSprite("Checkin/back_checkin_frame.png")
		:move(itemPosX,itemPosY)
		...
		--天数
		display.newSprite("Checkin/qdts"..(i+1)..".png")
		:move(itemPosX,itemPosY + 40)
...
--		:addTouchEventListener(self._btcallback)
    local btnCheckinVIPBg = cc.Scale9Sprite:create("public/lianglvbtn.png")
    btnCheckinVIPBg:setCapInsets(CCRectMake(20,10,66,27))
    btnCheckinVIPBg:setPosition(yl.WIDTH/2+ 200,112)
    btnCheckinVIPBg:setVisible(false)
    self:addChild(btnCheckinVIPBg)
    local newBtnSize = btnCheckinVIPBg:getContentSize()
	ccui.Button:create("Checkin/bt_checkin_vip.png","Checkin/bt_checkin_vip.png")
		:move(newBtnSize.width/2, newBtnSize.height/2)
		:setTag(CheckinLayer.BT_VIPGIFT)
		:addTo(btnCheckinVIPBg)
		:addTouchEventListener(self._btcallback)
    self._btVIPGift = btnCheckinVIPBg:getChildByTag(CheckinLayer.BT_VIPGIFT)

	--低保按钮	
	ccui.Button:create("Checkin/dibao.png","Checkin/dibao.png")
		:move(yl.WIDTH-300,112)
...
--		self._btVIPGift:loadTextureNormal("Checkin/bt_checkin_vip_0.png")
--		self._btVIPGift:loadTexturePressed("Checkin/bt_checkin_vip_0.png")
--	end
...
-- 设置签到按钮
function CheckinLayer:setCheckinBtn(enable, hide)
    if self.m_btnCheckinBg then
        self.m_btnCheckinBg:removeFromParent()
        self._btConfig = nil
    end
    local bg = enable and "public/lanlvbtn.png" or "public/graybtn.png"
    if bg then
        self.m_btnCheckinBg = cc.Scale9Sprite:create(bg)
        self.m_btnCheckinBg:setCapInsets(CCRectMake(20,10,66,27))
        self.m_btnCheckinBg:setPosition(yl.WIDTH/2,112)
        self:addChild(self.m_btnCheckinBg)
        if nil ~= hide then
            self.m_btnCheckinBg:setVisible(hide)
        end

        local newBtnSize = self.m_btnCheckinBg:getContentSize()
	    ccui.Button:create("Checkin/bt_checkin.png","Checkin/bt_checkin.png")
		    :move(newBtnSize.width/2, newBtnSize.height/2)
		    :setTag(CheckinLayer.BT_CHECKIN)
            :setEnabled(enable)
		    :addTo(self.m_btnCheckinBg)
		    :addTouchEventListener(self._btcallback)
        self._btConfig = self.m_btnCheckinBg:getChildByTag(CheckinLayer.BT_CHECKIN)
    end
end
...
print("onCheckinCallBack ===",result,message,subMessage)
	...
	if result == 10 and GlobalUserItem.bTodayChecked then
		self._scene:coinDropDownAni(function()
            self:setCheckinBtn(not GlobalUserItem.bTodayChecked)
...
--			ExternalFun.enableBtn(self._btConfig, true)
            self:setCheckinBtn(true)
		end
dump(GlobalUserItem.MemberList,"GlobalUserItem.MemberList",6)
print(GlobalUserItem.cbMemberOrder)
		if GlobalUserItem.MemberList[GlobalUserItem.cbMemberOrder] and 0 ~= GlobalUserItem.MemberList[GlobalUserItem.cbMemberOrder]._present then
			-- 领取送金
