...
function UserInfoLayer:ctor(scene)
    ...
    spriteContentBg:setContentSize(cc.size(800, 370))
    spriteContentBg:setPosition(yl.WIDTH/2, yl.HEIGHT/2 + 10)
    self:addChild(spriteContentBg)

	--标题
	display.newSprite("Information/title_info.png")
		:move(yl.WIDTH/2,yl.HEIGHT-130)
		:addTo(self)
	--返回
...
--		:addTo(self)
	...
	--self._txtBean:setPosition(870, 232)
...
--	    	:addTouchEventListener(btcallback)
    ...
    --   local wdtgmBg = display.newSprite("public/lanlvbtn.png")
	-- 	:move(850,150)
    --   :addTo(self)
    --   local newBtnSize = wdtgmBg:getContentSize()
	--    ccui.Button:create("Information/btn_qrcode_1.png","Information/btn_qrcode_1.png")
	...
	if GlobalUserItem.szMobilePhone == "" then
      ...
      --  local bdsjBg = display.newSprite("public/lianglvbtn.png")
		--    :move(500,150)
	    --	:addTo(self)
	    --ccui.Button:create("Information/bt_phone_bind.png","Information/bt_phone_bind.png")
		  --  :move(newBtnSize.width/2, newBtnSize.height/2)
		   -- :setTag(UserInfoLayer.BT_PHONE)
		   -- :addTo(bdsjBg)
		   -- :addTouchEventListener(btcallback)
	else
		-- 显示绑定的电话号码
		--local first = string.sub(GlobalUserItem.szMobilePhone,0,3)
		--local last = string.sub(GlobalUserItem.szMobilePhone,8,11)
		--local textId = string.format("绑定手机：%s****%s", first,last)
		--self._phone = cc.Label:createWithTTF(textId, "fonts/round_body.ttf", 25)
		--	:setTextColor(cc.c4b(176,204,243,255))
		--	:setAnchorPoint(cc.p(0,0.5))
		--	:setDimensions(345,60)
		--	:setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
...
--	local spriteSigneBg = display.newSprite("Information/info_frame_3.png")
--		:move(700,310)
--		:addTo(self)
--    local spriteSigneBgSize = spriteSigneBg:getContentSize()
--    spriteSigneBg:setContentSize(cc.size(spriteSigneBgSize.width - 500, spriteSigneBgSize.height))

    local spriteSigneBg = cc.Scale9Sprite:create("Information/info_frame_3.png")
        spriteSigneBg:setCapInsets(CCRectMake(40,10,479,54))
        spriteSigneBg:setContentSize(cc.size(480, 74))
        spriteSigneBg:move(820,300)
