...
local UndercarriageNotifyLayer = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.UndercarriageNotify")
...
ClientScene.BT_XINGYUNZP    = 22--
ClientScene.BT_QDYL         = 23--
ClientScene.BT_SHARE        = 24--
...
--	local head = HeadSprite:createClipHead(GlobalUserItem, 70)
	...
	--普通头像
--	local head = HeadSprite:createNormal(GlobalUserItem, 95)
--	head:setPosition(cc.p(self._btExit:getPositionX(), self._btExit:getPositionY()))
--	self._AreaTop:addChild(head)
--	self._head = head

	--宝箱动画
--	self.m_actBoxAni = ExternalFun.loadTimeLine("Lobby/BoxMovie.csb")
...
--	self._btBox:runAction(self.m_actBoxAni)
	...
	--if GlobalUserItem.szMobilePhone == "" then
		--showToast( self, "为了您的账户安全，请您绑定手机！！", 5 )
	--end
...
function ClientScene:onCreate()
    ...
    --
    -- 背景
    self._bg = ccui.ImageView:create("public/background_2.jpg")
        :move(display.center)
        :addTo(self)
 ...
 --   csbNode = csbNode:getChildByName("Image_bg")
	...
	--返回
	self._btExit = areaTop:getChildByName("Btn_Back")
	self._btExit:setTag(ClientScene.BT_EXIT)
	self._btExit:addTouchEventListener(btcallback)
    --玩家头像边框
    self.mHeadFrame = areaTop:getChildByName("Img_HeadFrame")
    --查看玩家信息
	self._btPersonInfo = areaTop:getChildByName("Btn_Person")
    ...
    self.mNickname = areaTop:getChildByName("Text_Nickname")
	self.mNickname:setString(nickname)

	----vip信息
   	self.mLevel = areaTop:getChildByName("Img_LvNum")
--    if GlobalUserItem.cbMemberOrder > 0 then
	    self.mLevel:loadTexture("Lobby/VIP"..GlobalUserItem.cbMemberOrder..".png")
 --   end
    --金币
    self._gold = areaTop:getChildByName("Text_Coin")
    local btn = areaTop:getChildByName("Btn_QuQian")
    btn:setTag(ClientScene.BT_BANK)
    btn:addTouchEventListener(btcallback)

    --游戏豆
    self._bean = areaTop:getChildByName("Text_Bean")
	btn = areaTop:getChildByName("Btn_ChongZhi")
    btn:setTag(ClientScene.BT_EXCHANGE)
      btn:setVisible(false)
    btn:addTouchEventListener(btcallback)
  self._bean:setVisible(false)

 local beannumbg = areaTop:getChildByName("Image_beannumbg")
    beannumbg:setVisible(false)

 local Imagebean = areaTop:getChildByName("Image_bean")
    Imagebean:setVisible(false)


    --公告
	self._btGonggao = areaTop:getChildByName("btn_gonggao")
	self._btGonggao:setTag(ClientScene.BT_GONGGAO)
	self._btGonggao:addTouchEventListener(btcallback)
self._btGonggao:setVisible(false)



    --设置
	self._btConfig = areaTop:getChildByName("Btn_Setting")
	self._btConfig:setTag(ClientScene.BT_CONFIG)
local posx=self._btConfig:getPositionX()
local posy=self._btConfig:getPositionY()
    self._btConfig:setPosition(self._btGonggao:getPosition())
	self._btConfig:addTouchEventListener(btcallback)

	
	--退出
    self._btQUIT= ccui.Button:create("Logon/closebtn.png","Logon/closebtn.png")
    self._btQUIT:setTag(ClientScene.BT_QUIT)
	self._btQUIT:setPosition(posx+5,appdf.HEIGHT-posy+43)
...
--    self.m_btnBox:setVisible(false)
--	local box = ExternalFun.loadCSB("Lobby/BoxMovie.csb", areaTop)
--	box:setPosition(self.m_btnBox:getPosition())
--	self._btBox = box
--    self._btBox:setVisible(false)

    --幸运转盘
	self._btXYZP = csbNode:getChildByName("btn_xyzp")
    self._btXYZP:setTag(ClientScene.BT_XINGYUNZP)
    self._btXYZP:addTouchEventListener(btcallback)
	...
	--分享
	btn = areaBottom:getChildByName("btn_share")
...
--	self.m_btBag:setVisible(infoShow)

--银行
	btn = areaBottom:getChildByName("Btn_Bank")
	btn:setTag(ClientScene.BT_BANK)
--	   btn:setPosition(self.m_btBag:getPosition())
	btn:addTouchEventListener(btcallback)
...
--	end
	--self:dismissPopWait()
	--self.mNickname:setString(GlobalUserItem.szNickName.."")
	--self.mLevel:loadTexture("Lobby/Top_LV_"..GlobalUserItem.wCurrLevelID..".png")
end
...
function ClientScene:onXYZP()
    self:onChangeShowMode(yl.SCENE_TABLE)
end
...
function ClientScene:onButtonClickedEvent(tag,ref)
    ...
    elseif tag == ClientScene.BT_GONGGAO then
        self:onGonggao()
    elseif tag == ClientScene.BT_XINGYUNZP then
        self:onXYZP()
...
function ClientScene:onChangeShowMode(nTag, param)
	...
	if bRoomList then
        ...
        self._btQUIT:setVisible(not var)
		self._btQUIT:setEnabled(not var)
	...
	elseif tag == yl.SCENE_ROOM then
        ...
        self._btQUIT:setVisible(true)
		self._btQUIT:setEnabled(true)
	...
	elseif tag == yl.SCENE_GAMELIST then
		...
		self._bg:loadTexture("public/background_2.jpg")
...
function ClientScene:getTagLayer(tag, param,curtag)
 ...
 --       dst:setLocalZOrder(-2000)
	...
	elseif tag == yl.SCENE_SHOPDETAIL then
		dst = ShopDetail:create(self, self._gameFrame, param)
    ...
    elseif tag == yl.SCENE_UNDERCARRIAGENOTIFY then
		dst = UndercarriageNotifyLayer:create(self)
    elseif tag == yl.SCENE_CONSIGNMENT_BUY then
		dst = ConsignmentBuyLayer:create(self)
    elseif tag == yl.SCENE_CONSIGNMENT then
		dst = ConsignmentLayer:create(self)
	elseif PriRoom then
