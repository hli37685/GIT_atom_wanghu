...
print("创建裁剪玩家头像 createClipHead",useritem,headSize,clippingfile)
	useritem = HeadSprite.checkData(useritem)
	...
	--clipSp = cc.Sprite:create("public/graybtn.png")
	if nil ~= clipSp then
		--裁剪
		...
		sp:addChild(clip)
--print("创建裁剪玩家头像 createClipHead",clipSp,clip,sp)
		return sp
	end
...
--print("frameparam._framefile",frameparam._framefile,framestr,framefile,scaleRate,zorder)
	self:updateHeadFrame(framefile, scaleRate):setLocalZOrder(zorder)
end

--更新头像框
function HeadSprite:updateHeadFrame(framefile, scaleRate)
--print("updateHeadFrame",framefile,scaleRate)
--framefile="head_mask.png"
	local spFrame = nil	
...
--		local positionRate = self.m_tabFrameParam._posPer or cc.p(0.5, 0.64)
        local positionRate = self.m_tabFrameParam._posPer or cc.p(0.5, 0.52)
